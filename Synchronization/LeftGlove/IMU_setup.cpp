...
void initFingerChannel(FingerChannel& fc, bool (&status)[4]) {
  ...
  // testConnection() is informational only — do not gate DMP on it
  status[0] = IMU_MID.testConnection();
  status[2] = IMU_PROX.testConnection();
  ...
  // Gated on dmpInitialize() success, NOT testConnection()
  if (devStatus1 == 0) {
    t = millis();
    IMU_MID.CalibrateAccel(6);
    IMU_MID.CalibrateGyro(6);
