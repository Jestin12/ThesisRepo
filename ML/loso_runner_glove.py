...
ap.add_argument('--gesture_set', default='static', choices=['static', 'dynamic'],
                help='Which gesture set + folder layout to use. '
                     '"static" reads <subject>/Static/<gesture>/*.csv with the Static '
                     'gesture vocabulary (Double_Okay, Double_Phone, ...). '
                     '"dynamic" reads <subject>/Dynamic/<gesture>/*.csv with the Dynamic '
                     'gesture vocabulary (Double_Lower, Double_Pistol_Recoil, ...).')
...
# Dynamic-gesture class set (matches loso_runner.py for the CNN/LSTM runs).
DYNAMIC_GESTURE_LABELS_ALL = [
    'Double_Lower', 'Double_Nothing', 'Double_Pistol_Recoil',
    'Double_Raise', 'Double_Wiggle', 'Double_cmere', 'Drum_Roll',
]
GESTURE_LABELS_ALL = (DYNAMIC_GESTURE_LABELS_ALL if args.gesture_set == 'dynamic'
                     else STATIC_GESTURE_LABELS_ALL)
GESTURE_LABELS = GESTURE_LABELS_ALL if args.include_nothing else \
                 [g for g in GESTURE_LABELS_ALL if g != 'Double_Nothing']
DATA_SUBDIR = 'Dynamic' if args.gesture_set == 'dynamic' else 'Static'
...
print(f'  gestures: {args.gesture_set} ({DATA_SUBDIR}/)')
...
# ── Subject discovery ────────────────────────────────────────────────────────
def discover_subjects(data_root, gesture_labels, subdir):
    out = []
    for sub in sorted(os.listdir(data_root)):
        if sub in ('Combined', 'Copy', 'tree.txt'):
            continue
        gd = os.path.join(data_root, sub, subdir)
        if not os.path.isdir(gd):
            continue
        ok = True
        for g in gesture_labels:
            if not glob.glob(os.path.join(gd, g, '*.csv')):
                ok = False; break
        if ok:
            out.append(sub)
    return out

SUBJECTS = discover_subjects(args.data_root, GESTURE_LABELS, DATA_SUBDIR)
...
def load_subject(subject_dir, sensor_cols, gesture_labels, subdir):
    """Returns list of (T, C) float32 arrays plus parallel list of labels."""
    trials, labels = [], []
    for g in gesture_labels:
        for fpath in sorted(glob.glob(os.path.join(subject_dir, subdir, g, '*.csv'))):
            try:
...
for sub in SUBJECTS:
    raw_trials, raw_labels = load_subject(
        os.path.join(args.data_root, sub), SENSOR_COLS, GESTURE_LABELS, DATA_SUBDIR)
...
cfg_rows = [
    ('Gesture set', f'{args.gesture_set} ({DATA_SUBDIR}/ folder)'),
