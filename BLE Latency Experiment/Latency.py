...
pending_start = None      # time when last GET was sent
...
def handle_notification(sender, data: bytearray):
    """Called when ESP32 sends a notification."""
    global buffer, pending_start
...
async def run():
    ...
    async with BleakClient(device) as client:
        ...
        print("Notifications started. Measuring latency... (Ctrl+C to stop)")
        ...
        try:
            ...
            while True:
                ...
                # 2) Wait for response (notification handler clears pending_start)
                while pending_start is not None:
                    await asyncio.sleep(0.001)

                count += 1
                # 3) Wait a bit before next measurement
